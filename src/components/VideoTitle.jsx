        ...
        <>
            <div className="absolute w-[99%] px-12 md:px-28 aspect-video pt-[40%] md:pt-[20%] bg-gradient-to-r from-black">
                ...
                <div className="flex h-auto gap-2 pt-2">
                    <Link to={`https://www.youtube.com/watch?v=${trailerVideo?.key}`} target="_black" className='px-3 py-2 text-lg text-center text-black bg-white rounded-md md:w-32 hover:bg-opacity-80 bg-opacity-90'> ▶Play</Link>
                    <Link to={`https://www.themoviedb.org/movie/${tmdbId}`} target="_blank" className='px-3 py-2 text-lg text-center text-white bg-opacity-50 rounded-md md:w-32 bg-gray-50'>ℹ More Info</Link>
                </div>
            </div>
        </>
    )
